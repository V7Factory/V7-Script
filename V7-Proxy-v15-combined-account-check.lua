...
local function checkAccountID(var)
    if licenseChecked or var[0] ~= "OnDialogRequest" or not var[1] then
        return false
    end

    local dialog = var[1]
    if not dialog:find("account id is", 1, true) then
        return false
    end

    local uid = tonumber(dialog:match("account id is%s+`w#(%d+)"))
        or tonumber(dialog:match("account id is%s+#(%d+)"))
        or tonumber(dialog:match("account id is%s+(%d+)"))
    ...
    if uid == uid_1 or uid == uid_2 then
        ...
        overlay("Have problem? DM Discord `2@kevin.sinaga")
        RemoveHook("account_check")
    else
...
AddHook("OnVarlist", "account_check", checkAccountID)

sendPacket(2, "action|input\n|text|`0Proxy by `cV7-Factory")
sleep(100)

local localPlayerForCheck = getLocal()
if localPlayerForCheck and localPlayerForCheck.netId then
    sendPacket(2, "action|wrench\n|netid|" .. localPlayerForCheck.netId)
end
